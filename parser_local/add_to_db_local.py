...
def add_to_db(product_name, product_price_original, product_price, product_price_with_ozon_card, product_images,
    ...
    # parser_requests_local.info("affected rows = {}".format(cursor.rowcount))

    if format(cursor.rowcount) == 0:
        print('Товар не добавлен (скорее всего он уже есть в базе данных): ' + product_article)
    else:
        print('Добавлен товар: ' + product_name)
